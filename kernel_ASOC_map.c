...
__kernel void Mapping(
                      ...
                      __global   float   *SAVETAU,  // 16 optical-depth or column-density image
                      const      int      SAVE_COLDEN // 17 >0 => save column density, not tau
#if (ROI_MAP>0)
                      ,constant   int     *ROI     // 18 [x0, x1,y0, y1, z0, z1] limits of ROI
#endif
                     )
{
...
# endif      
      ...
      TAU    += DTAU ;
#if 1
      if (SAVE_COLDEN>0) colden += sx*dens ;
#else
      // column density counting only cells at a certain level
      // IMF512_000448.soc has 7 levels = 0,1,2,3,4,5,6
      if (SAVE_COLDEN>0) {
         if (olevel==3) colden += sx*dens ;
      }
#endif
   ...
   if (SAVE_COLDEN) SAVETAU[id] = colden * LENGTH ;
   else             SAVETAU[id] = TAU ;
...
__kernel void HealpixMapping(
                             ...
                             __global float     *SAVETAU, // 16 TAU ... or column density
                             const      int     SAVE_COLDEN  // 17  if >0, save column density, not tau
#if (ROI_MAP>0)
                             ,constant   int    *ROI     // 18 [x0,x1,y0,y1,z0,z1] includive limits of ROI
#endif
                            )
{
   const int id   = get_global_id(0) ;   // one work item per map pixel
   ...
   if (SAVE_COLDEN>0)   SAVETAU[id] = colden*LENGTH ;
   else                 SAVETAU[id] = TAU ;
}
