...
# 2019-12-16  since julia does not support AopenCL, use pyOpenCL via pyCall
# 2019-12-25  ??? previus versoin was not working, now SHG is ok
using Printf, Mmap
using DelimitedFiles, Statistics
using PyCall
cl = pyimport("pyopencl")
np = pyimport("numpy")
...
# solve using GPU
function InitCL(;GPU=0, platforms=[], sub=0)
  """
  Usage:
     platform, device, context, queue, mf = InitCL(GPU=0, platforms=[])
  Input:
  ...
  """
  platform, device, context, queue = nothing, nothing, nothing, nothing
  possible_platforms = range(1,stop=6)
  if (length(platforms)>0)
    possible_platforms = platforms
  end
  device = []
  for iplatform in possible_platforms
    @printf("---------- try platform %d ...\n", iplatform)
    try
      ...
      @printf("---------- platform %d not acceptable\n", iplatform)  
    end
  end
...
prog        =  cl.Program(context, source)
println("---------- prog= ", prog)
program     =  prog.build(OPT)
...
A_buf       =  cl.Buffer(context, mf.READ_WRITE, BATCH*(Int(floor((NE*NE-NE)/2)))*4) # lower triangle only
...
# DoSolve.set_scalar_arg_dtypes([np.int32, np.int32, np.None, np.None, np.None, np.None, np.None, np.None, np.None, np.None, np.None, np.None, np.None ])
#                              batch     isize     Iw       L1       L2       Tdown    Ea      
DoSolve.set_scalar_arg_dtypes([np.int32, np.int32, nothing, nothing, nothing, nothing, nothing, 
#                              Ibeg     AF       AABS     AEMIT    LL       [XX]
...
function solve_with_tree(T, ICELLS, INT, isize)
  ...
  println("---------- queue  ", queue)
  println("---------- AF_buf ", AF_buf)
  cl.enqueue_copy(queue, AF_buf,   AF)      # AF
  noIw  =  read(FP, Int32)
  Iw    =  zeros(Float32, noIw)
  read!(FP, Iw)
  cl.enqueue_copy(queue, Iw_buf, Iw)        # Iw
  L1    =  zeros(Int32, NE*NE)
  read!(FP, L1)
  cl.enqueue_copy(queue, L1_buf,  L1)       # L1
  L2    =  zeros(Int32, NE*NE)
  read!(FP, L2)
  cl.enqueue_copy(queue, L2_buf,  L2)       # L2
  Tdown =  zeros(Float32, NE)
  read!(FP, Tdown)
  cl.enqueue_copy(queue, Tdown_buf, Tdown)  # Tdown
  EA    =   zeros(Float32, NE*NFREQ)
  read!(FP, EA)
  cl.enqueue_copy(queue, EA_buf,    EA)     # EA
  Ibeg  =   zeros(Int32, NFREQ)
  read!(FP, Ibeg)
  cl.enqueue_copy(queue, Ibeg_buf,  Ibeg)   # Ibeg
  cl.finish(queue)
  ...
  cl.finish(queue)
  DoSolve(queue, GLOBAL, LOCAL,  Int32(batch),   Int32(isize-1),  Iw_buf,    L1_buf,  L2_buf,   Tdown_buf, EA_buf, 
...
function solve_with_hash(isize, ABSORBED, EMITTED)
  ...
  for icell = 1:CELLS        
    ...
    elseif (SOLVED[HASH[icell]]==1)                      # hash is already queued, result not available
      ...
      if (unsolved==BATCH)                               # buffer is full, solve with a single call to CL kernel
        ...
        cl.enqueue_copy(queue, ABS_buf, tmp)
        cl.finish(queue)
        DoSolve(queue, GLOBAL, LOCAL,  Int32(BATCH),   Int32(isize-1),  
  ...
  if (unsolved>0) # solve the last partial batch -- last cells for which Dict did not give an answer
    cl.enqueue_copy(queue, ABS_buf, tmp)
    cl.finish(queue)
    DoSolve(queue, GLOBAL, LOCAL,  Int32(unsolved),   Int32(isize-1),  Iw_buf,    L1_buf,  L2_buf,   Tdown_buf, EA_buf, 
    Ibeg_buf,  AF_buf,  ABS_buf,  EMIT_buf,  A_buf,   X_buf)
    cl.finish(queue)
    tmp[:]  =  cl.read(queue, EMIT_buf)        # BATCH*NFREQ
    for iun = 1:unsolved
...
function process_size_stochastic(isize)
  ...
  cl.enqueue_copy(queue, AF_buf, np.asarray(AF, np.float32))
  ...
  for icell in 1:BATCH:CELLS
    ...
    cl.enqueue_copy(queue, emit, EMIT_buf)
