...
if (USER.USE_EMWEIGHT>0):
    ...
    if (USER.USE_EMWEIGHT==2):
        print("================================================================================")
        print("EMWEIGHT==2")
        print(" ARE YOU SURE THIS OPTION IS WORKING IN SOCS ?????")
        print(" TEST IT FIRST !!!!")
        print("================================================================================")
        time.sleep(20)
...
-D OPT_IS_HALF=%d" % \
(  NX, NY, NZ, USER.DSC_BINS, USER.WITH_ALI, int(USER.PS_METHOD),
   ...
   USER.OPT_IS_HALF)
###
NVARGS  = " "     # fastest option for NVIdia GPU ?
if (LOCAL<16):
    NVARGS  =   " -cl-fast-relaxed-math"  # fastest option for Intel CPU ?
    # OR BUGGY INTEL or POCL ???
    # NVARGS  =   " -cl-opt-disable"
...
source            =  INSTALL_DIR+"/kernel_SOC_sca.c"        
...
kernel_P.set_scalar_arg_dtypes(
#   0           1          2            3       4    
...
for II in range(3):  # loop over PSPAC, BGPAC and DFPAC==DIFFUSERAD simulations
    ...
    if (II==0):   # point source
        # 2017-12-25 -- each work item does BATCH packages, lopping over the sources
    ...
    for IFREQ in range(NFREQ):
        ...
        if ((II==2)&(USER.USE_EMWEIGHT>0)):
            print("UPDATE EMWEI - for diffuse radiation")
            skip += 1 
            if (skip==3):
                skip = 0
        ...
        if (II==2): # DFPAC --- the same kernel as for CLPAC !!
            ...
            if (USER.USE_EMWEIGHT==2):
                ...
                while 1:
                    ...
                    no     =  len(m[0])
                    loops +=  1
                    print("loops %3d,   cells %6d" % (loops, no))
                    ...
                    kernel_CL(commands[ID], [8192,], [LOCAL,],
                    np.int32(2),  CLPAC, BATCH, seed, ABS_buf[ID], SCA_buf[ID],
                    LCELLS_buf[ID], OFF_buf[ID], PAR_buf[ID], DENS_buf[ID], EMIT_buf[ID], DSC_buf[ID], CSC_buf[ID], 
                    NODIR, ODIR_buf[ID], USER.NPIX, USER.MAP_DX, USER.MAPCENTRE, RA_buf[ID], DE_buf[ID], OUT_buf[ID],
                    OPT_buf[ID], ABU_buf[ID], EMWEI_buf[ID], EMINDEX_buf[ID])
                    commands[ID].finish()
                ...
                # kernel_CL(commands[ID], [GLOBAL,], [LOCAL,],
                # np.int32(II), PACKETS, BATCH, seed, ABS, 
                # SCA, LCELLS_buf[ID], OFF_buf[ID], PAR_buf[ID], DENS_buf[ID], 
                # EMIT_buf[ID], DSC_buf[ID], CSC_buf[ID], NODIR, ODIR_buf[ID], 
                # USER.NPIX, USER.MAP_DX, USER.MAPCENTRE, RA_buf[ID], DE_buf[ID], 
                # OUT_buf[ID], OPT_buf[ID], ABU_buf[ID], EMWEI_buf[ID], EMINDEX_buf[ID])
            ...
            if ((II==1)&(size(HPBG)>0)): # using a healpix map for the background
                ...
                kernel_P(commands[ID], [GLOBAL,], [LOCAL,], 
                np.int32(PACKETS),  np.int32(BATCH), np.float32(seed),  ABS_buf[ID],  SCA_buf[ID], 
                PSPOS_buf[ID], PS_buf[ID], 
                LCELLS_buf[ID], OFF_buf[ID], PAR_buf[ID], DENS_buf[ID], DSC_buf[ID], CSC_buf[ID], 
                NODIR, ODIR_buf[ID], USER.NPIX, np.float32(USER.MAP_DX), USER.MAPCENTRE, 
                RA_buf[ID], DE_buf[ID], OUT_buf[ID], ABU_buf[ID], OPT_buf[ID], 
                XPS_NSIDE_buf[ID], XPS_SIDE_buf[ID], XPS_AREA_buf[ID])
...
if (CLPAC%CELLS!=0):
    print("Should CLPAC not be multiple of CELLS?? CELLS=%d,CLPAC=%d", CELLS, CLPAC)
...
if (CLPAC>0):
    ...
    for IFREQ in range(NFREQ):     # loop over single frequencies (all the frequencies)
        ...
        if (USER.USE_EMWEIGHT>0):
            skip += 1 
            if (skip==3):
                print("UPDATE EMWEI -- for emitted.data")
                skip = 0
                tmp      =  asarray(EMITTED[:,IFREQ-REMIT_I1].copy(), float64)
                if (1):
                    mmm      =  nonzero(~isfinite(tmp))
                ...
                if (USER.USE_EMWEIGHT==2):
                    ...
                    ###
                    if (1):
                        print("EMPAC  %12.4e .... %12.4e" % (min(EMPAC), max(EMPAC)))
                commands[ID].finish()
                cl.enqueue_copy(commands[ID], EMWEI_buf[ID], EMWEI)
